...
function dangnhap(event) {
    try {
        ...
        // Lặp qua danh sách tài khoản để kiểm tra
        arr.forEach(element => {
            // Nếu là tài khoản mặc định
            if (defaultUsername === element.username && defaultPassword === element.password) {
                // Kiểm tra quyền và điều hướng
                if (element.authority === "admin") {
                    found = true;
                    localStorage.setItem("adminSignedin", defaultUsername);
                    localStorage.setItem("isSignedin", "true");
                    window.location.href = "/admin.html";
                    event.preventDefault(); // Ngăn reload
                } else if (element.authority === "user") {
                    localStorage.setItem("isSignedin", "true");
                    localStorage.setItem("userSignIn", defaultUsername);
                    localStorage.setItem("currentUser", JSON.stringify(element));
                    found = true;
                }
            }
